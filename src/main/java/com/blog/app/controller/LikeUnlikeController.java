...
public class LikeUnlikeController {
  ...
  public Map<String, Integer> getPostLikeUnlikeCount(@PathVariable("postId") int postId,
    ...
    if (authentication != null) {
      CurrentlyLoggedInUser currentlyLoggedInUser = (CurrentlyLoggedInUser) authentication
          .getPrincipal();
      LikeUnlikeDomain domain = this.service
          .getByPostIdAndUserId(postId, currentlyLoggedInUser.getUserId());
      if (domain != null) {
        map.putIfAbsent("userAction", domain.getLikeUnlike());
      }
    }
    return map;
  }
